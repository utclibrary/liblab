    ...
    return this.each(function () {
      ...
      $this.keyup(function (e) {
        ...
        if ([38, 40, 13].indexOf(e.keyCode) == -1 && (e.keyCode != 8 ? $this.val().length >= $this.opts.min_chars : true)) {
          ...
          // handle spacing misspelling
          /*switch (q) {
            case 'pro quest':
            case 'proquest':{
              q = 'proquest';
              $(".dbCard:contains('ProQuest Central\n')").insertBefore('.dbCard:first');
              break;
            }
            case 'jstor':
              $(".dbCard:contains('JSTOR\n')").insertBefore('.dbCard:first');
              break;
            case 'one file':
              q = 'onefile';
              break;
            case 'science direct':
              ...
              break;
            case 'science':
              $(".dbCard:contains('Science (Gale OneFile)')").insertBefore('.dbCard:first');
              break;
          }*/
          $list.children($this.opts.ignore.trim() ? ":not(" + $this.opts.ignore + ")" : '').removeClass('selected').each(function () {
            ...
            var data = (
              ...
              $(this).text()
            ).toLowerCase().replace(/subject(s\b|\b):.*/gi, "");
...
jQuery.fn.highlight = function (t) {
  function e(t, i) {
    ...
    } else if (1 == t.nodeType && t.childNodes && !/(script|style)/i.test(t.tagName) && !/ul/i.test(t.tagName))
