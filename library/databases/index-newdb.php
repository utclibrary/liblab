...
// connect to database
require_once $_SERVER['DOCUMENT_ROOT'].'/includes/dbconnect-new.php';
// declare variables
...
$queryKey = $queryKeySubj = $outputSLA = $outputLG = $h1Prepend = $contentType = $queryContentType = $queryKeyAlpha = $displayAlpha = $lgURL = $lgDescription = $subjectGroup = "";
if (isset($_GET["type"])) {
...
if ($alpha === "ALL") {
  ...
  ";
    ...
    $resultSLA = mysqli_query($conDBofDBs, "set names 'utf8'");
    $resultSLA = mysqli_query($conDBofDBs, $querySubjectListAlpha) or die($error);
...
FROM DBofDBs.Dbases
LEFT JOIN DBofDBs.DBRanking
ON Dbases.Key_ID = DBRanking.Key_ID
LEFT JOIN DBofDBs.SubjectList
...
$alphaList = mysqli_query($conDBofDBs, "set names 'utf8'");
$alphaList = mysqli_query($conDBofDBs, $queryAlpha) or die($error);
...
$resultSL = mysqli_query($conDBofDBs, "set names 'utf8'");
$resultSL = mysqli_query($conDBofDBs, $querySubjectList) or die($error);
...
$resultTL = mysqli_query($conDBofDBs, "set names 'utf8'");
$resultTL = mysqli_query($conDBofDBs, $queryTypeList) or die($error);
...
<?php
            ...
            ?>
		            <div class="featureBox">
		              ...
		              <hr class="featureHR">
                  <?php
            $randquery = "SELECT Dbases.Title, Dbases.Key_ID, Dbases.ShortDescription, Dbases.ContentType, Dbases.HighlightedInfo, Dbases.SimUsers, Dbases.ShortURL, Dbases.New, Dbases.Trial, Dbases.TutorialURL FROM Dbases
							  WHERE Dbases.CANCELLED = 0 AND Dbases.MASKED = 0 AND Dbases.Key_ID <> 529 ORDER BY RAND() LIMIT 1";
            $result = mysqli_query($conDBofDBs, $randquery) or die($error);
...
<?php
$multiquery = "SELECT Dbases.Title, Dbases.Key_ID, Dbases.ShortDescription, Dbases.ContentType, Dbases.HighlightedInfo, Dbases.SimUsers, Dbases.ShortURL, Dbases.TutorialURL, Dbases.New, Dbases.Trial FROM Dbases INNER JOIN DBRanking ON DBRanking.Key_ID = Dbases.Key_ID INNER JOIN SubjectList ON DBRanking.Subject_ID = SubjectList.Subject_ID WHERE SubjectList.SubjectCode = 'MULTI' AND DBRanking.TryTheseFirst = 0 AND Dbases.CANCELLED = 0 AND Dbases.MASKED = 0 ORDER BY DBRanking.Ranking";
        $resultMulti = mysqli_query($conDBofDBs, $multiquery) or die($error);
...
<?php
$newquery = "SELECT Dbases.Title, Dbases.Key_ID, Dbases.ShortDescription, Dbases.ContentType, Dbases.HighlightedInfo, Dbases.SimUsers, Dbases.TutorialURL, Dbases.ShortURL FROM Dbases WHERE Dbases.New = 1 ORDER BY Dbases.Title";
        $result = mysqli_query($conDBofDBs, $newquery) or die($error);
        if (!mysqli_num_rows($result)) {
...
$query = "SELECT Dbases.Title, Dbases.NotProxy, Dbases.Key_ID, Dbases.ShortDescription, Dbases.ContentType, Dbases.HighlightedInfo, Dbases.TutorialURL, Dbases.SimUsers, Dbases.ShortURL, Dbases.DropDate, Dbases.New, Dbases.Trial, DBRanking.TryTheseFirst, SubjectList.NotSubjectList, SubjectList.LibGuidesPage,
GROUP_CONCAT( DISTINCT '<li>' , IF (SubjectList.NotSubjectList=0, SubjectList.Subject, null) , '</li>' ORDER BY SubjectList.Subject SEPARATOR '') AS Subjects,
...
(SELECT SubjectGroup.GroupName From SubjectGroup WHERE SubjectGroup.Group_ID = DBRanking.ParentGroup_ID) as Parent
          ...
          ORDER by ".$orderby;
         //echo "<pre>".$query."</pre>";
$result = mysqli_query($conDBofDBs, "set names 'utf8'");
$result = mysqli_query($conDBofDBs, $query) or die($error);
if (!mysqli_num_rows($result)) {
    echo "No results";
} else {
    ...
    <div class="col-lg-6">
      ...
      <?php
            ...
            //headings for subject posix_getgroups
            if ((!empty($row['GroupName']))&&($subj != "A to Z")) {
                if ((!empty($row['Parent']))&&($subj != "A to Z")) {
                    $currentSubjectGroup .= $row['Parent'].": ";
                }
                $currentSubjectGroup .= $row['GroupName'];
                if ($currentSubjectGroup != $subjectGroup){
                  echo '<h2 class="subjectGroup"style="display: block !important;">'.$currentSubjectGroup.'</h2>';
                }
                $subjectGroup=$currentSubjectGroup; $currentSubjectGroup = "";
            }
