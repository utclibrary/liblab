...
else
{
	...
	// fetch all start dates
	$resultDate = mysqli_query($conDates , "SELECT startDate, termLabel FROM Calendar ORDER BY startDate");
	...
	// fetch open and close times for current day of week
	$resultDate = mysqli_query($conDates , "SELECT $start, $end FROM Term WHERE termLabel='$term'");
	while($row = mysqli_fetch_array($resultDate))
	{
...
//		$display .= date('l, F jS',$today) . ", CLOSED";
	...
	// otherwise set display hours
		...
		$display .= " " . date('g:ia',$open) . "-" . date('g:ia',$close);

	//$display .= "</a></div>";

	mysqli_close($conDates);
