...
//eliminate SubjectList.Format = 0 to pull in all groups (ex: multiusbject, ebooks, etc.)
	$result1 = mysqli_query($conLuptonDB , $query1);
	$count = 0;
...
echo "<label class='limitbysubject hide' style='display:none' aria-label='limit by subject' for='databaseformat'>Limit by Subject (optional)</label>
...
foreach ($dbFormatKey as $key => $i){
  ...
  $result3 = mysqli_query($conLuptonDB , $query3);

  if (!$conLuptonDB || empty($result3))
    {
	...
	$result2 = mysqli_query($conLuptonDB , $query2);

	if (!$conLuptonDB || empty($result2))
		{
			echo "Database search is currently unavailable.";
		}
		else
...
<form class='form-search' name='databases' action='https://liblab.utc.edu/scripts/LGForward.php' method='get' target='_blank'>
	...
	<label for="jumptodatabase" class="hide" aria-label="select databases from full list">Select Database</label>
			<select id="jumptodatabase" name='db0' class='selectdb input-xxlarge jumptodatabase' onChange="changetext('jumptodatabase');">
					<option></option>
	<option value='529'>- A to Z Database List - </option>
...
?>
	...
	<ul id="db-legend">
		<li><span class='icon-th icon-large'><span class='hidden'>data</span></span>Data</li>
