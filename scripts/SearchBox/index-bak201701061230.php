...
<div id='myTabContent' class='tab-content'>
	<div class='tab-pane active' id='everything'>
			<form aria-label="quick search form" class='form-search' name='everything' action='http://utc.on.worldcat.org/search' method='get' target='_blank'>
...
foreach ($dbFormatKey as $key => $i){
      ...
      $("a#lgSubjBtn").replaceWith("<a id='lgSubjBtn' class='btn' href='http://guides.lib.utc.edu/' target='_blank'>Subject Guides</a>");
			$('#resetDb').hide();
      ...
      <?php foreach ($dbFormatKey as $key => $i){ ?>
        ...
        if(formatValue == <?php echo $dbFormatKey[$i]; ?>){
          $("a#lgSubjBtn").replaceWith("<a id='lgSubjBtn' class='lgSubjBtn btn-primary btn' href='http://guides.lib.utc.edu/<?php echo $dbLGpage[$i] ?>' target='_blank'><?php echo $dbFormatName[$i]; ?> Subject Guide</a>");
  ...
  if (!$con || empty($result3))
      ...
      while($row = mysql_fetch_array($result3))
    {
      ...
      if (($row['OpenAccess']) == '1')
      $dbDescrip[$keyID] .= "<a  id='oa_icon' title='open access resource' href='http://guides.lib.utc.edu/openaccess/overview' target='_blank'><img src='//www.utc.edu/library/_resources/icon-oa.svg' alt='open access icon: an open padlock'></img></a>";
...
$( "span:contains('Ebooks: ')" ).parent('p').before( "<span class='icon-book icon-large db-legend' title='Ebooks' onclick='showModal();'></span>" );
$( "span:contains('Index: ')" ).parent('p').before( "<span class='icon-list-alt icon-large db-legend' title='Index' onclick='showModal();'></span> " );
$("span").filter(function() {return $(this).text() == "Fulltext: ";}).parent('p').before( "<span class='icon-file-text icon-large db-legend' title='Fulltext' onclick='showModal();'></span> " );
$( "span:contains('Partial Fulltext: ')" ).parent('p').before( "<span class='icon-file-alt icon-large db-legend' title='Partial Fulltext' onclick='showModal();'></span> " );
$( "span:contains('Streaming Video: ')" ).parent('p').before( "<span class='icon-film icon-large db-legend' title='Streaming Video' onclick='showModal();'></span> " );
$( "span:contains('Streaming Music: ')" ).parent('p').before( "<span class='icon-music icon-large db-legend' title='Streaming Music' onclick='showModal();'></span> " );
$( "span:contains('Images: ')" ).parent('p').before( "<span class='icon-picture icon-large db-legend' title='Images' onclick='showModal();'></span> " );
		}
  </script>
...
echo "<label class='limitbysubject hide' aria-label='limit by subject' for='databaseformat'>Limit by Subject</label>
...
foreach ($dbFormatKey as $key => $i){
    ...
    ${"libGuideSub".$i}="
    <a class='btn btn-block lgbtn' href='http://guides.lib.utc.edu/".$dbLGpage[$i]."'
...
<!--
<a data-toggle="tooltip" title="Not sure where to start? Explore databases grouped by subject"class="btn btn-mini search-xtra-left" href="http://guides.lib.utc.edu/eresources" target="_blank">Databases By Subject</a>
-->

<a id="lgSubjBtn" class="btn" href="http://guides.lib.utc.edu/" target="_blank">Subject Guides</a>
  ...
  <div class="accordion-group">
    ...
    <div id="collapseOne" class="accordion-body collapse">
      <div class="accordion-inner">
        ...
        {
          echo "<li>
          <a href='http://www5.lib.utc.edu/databases/LGForward.php?db=".$row['Key_ID']."' target='_blank'>";
...
<a  class="btn btn-mini search-xtra-right" href="http://guides.lib.utc.edu/" target="_blank">Subject Guides</a>
-->
<?php if ($today < $endTime) echo $emergMessage; ?>
</div>
